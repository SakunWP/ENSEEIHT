...
for i = 1:length(EbN0_db)
 ...
 bits_recu_hard=pskdemod(demodulation,4,pi/4,OutputType='bit'); % !! du coup il faut modifier la décision ici
 %je le fais ci-dessous :
 ...
 % Calcul des TEB
 %erreurs_soft =(bits(1:length(decoded_soft)) ~= decoded_soft); % Comparaison bit à bit
 % TEB_soft(i) = sum(erreurs_soft) / length(decoded_soft);
 % 
 % erreurs_hard = (bits(1:length(decoded_hard)) ~= decoded_hard);
 % TEB_hard(i) = sum(erreurs_hard) / length(decoded_hard);
