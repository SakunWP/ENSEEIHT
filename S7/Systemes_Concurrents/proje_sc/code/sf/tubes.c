...
typedef struct _tube {
   ...
   Condition pasPlein; //pour permettre l'acces à l'ecriture
...
int tubeOuvrir(INoeud * iNoeud, Fichier * f, uint16_t fanions, uint16_t mode)
{
   ...
   //exclusionMutuelleEntrer(&(tube->verou));
...
size_t tubeEcrire(Fichier * f, void * buffer, size_t nbOctets)
{
   ...
   do {
      ...
      while (tube->taille == MANUX_TUBE_CAPACITE){
       conditionDiffuser(&(tube->pasVide)); //On préviens les potentiel lecteurs
   ...
   // condition de sortie est bloquant -> tant qu'on n'a pas tout écis on ne sort pas

   conditionDiffuser(&(tube->pasVide)); // important de mettre avant mutex (logique)
   exclusionMutuelleSortir(&(tube->verrou)); 
   rintk_debug(DBG_KERNEL_TUBE, "out\n");
...
int sys_tube(ParametreAS as, int * fds)
{
   ...
   // initialisation exclusion mutuelle
   exclusionMutuelleInitialiser(&(tube->verrou));

   //initialisation des condition d'acceptation
