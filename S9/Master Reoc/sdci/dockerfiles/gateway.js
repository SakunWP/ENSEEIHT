...
var DROP_RATE = typeof argv.drop_rate === 'number' ? argv.drop_rate : 0.1;
...
app.post('/devices/register', function(req, res) {
 ...
 app.post('/device/:dev/data', function(req, res) {
    ...
    if (shouldDrop()) {
        console.log('DROP: message from device ' + dev + ' dropped (rate=' + DROP_RATE + ')');
        res.status(E_OK).send({ dropped: true, message: 'Dropped by gateway' });
        return;
    }
    doPOST(
